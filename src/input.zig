...
pub fn inputToNote(input: []u8, allocator: std.mem.Allocator, main: f64) ![]Note {
    ...
    for (input) |c| {
        switch (c) {
            'A' => {
                const note = Note.init(.triangleWave, main);
                try noteList.append(note);
            },
            'B' => {
                const note = Note.init(.squareWave, types.getNoteFactor(main, 2));
                try noteList.append(note);
            },
            'C' => {
                const note = Note.init(.squareWave, types.getNoteFactor(main, 3));
                try noteList.append(note);
            },
            'D' => {
                const note = Note.init(.squareWave, types.getNoteFactor(main, 5));
                try noteList.append(note);
            },
            'E' => {
                const note = Note.init(.squareWave, types.getNoteFactor(main, 6));
                try noteList.append(note);
            },
            'F' => {
                const note = Note.init(.squareWave, types.getNoteFactor(main, 8));
                try noteList.append(note);
            },
            'G' => {
                const note = Note.init(.squareWave, types.getNoteFactor(main, 10));
                try noteList.append(note);
            },
