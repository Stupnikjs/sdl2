...
pub fn SDL_PlayBuffer(buf: []u8, spec: SDL.SDL_AudioSpec, len: u32) !void {
    _ = len;
    ...
    const queue_result = SDL.SDL_QueueAudio(device_id, buf.ptr, 2); // Example size: 2 bytes per sample (stereo 16-bit)
    if (queue_result != 0) {
